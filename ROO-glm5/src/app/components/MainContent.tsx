...
import { sendChatMessage, isConnected, disconnectGateway } from '../../services/openclawGateway';
...
export function MainContent({ onAddTask, currentTaskId, selectedMenu, tasks, onUpdateTaskTitle, onUpdateTaskStatus }: MainContentProps) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // 使用 sendChatMessage 发送消息并处理流式响应
      const response = await sendChatMessage(
        messageContent,
        taskIdToUse || undefined,
        (streamText: string) => {
          // 流式文本更新
          console.log('[MainContent] Stream update:', streamText.substring(0, 50));
          if (taskIdToUse) {
            setMessagesMap(prev => {
              const messages = prev[taskIdToUse!] || [];
              return {
                ...prev,
                [taskIdToUse!]: messages.map(msg =>
      ...
      // 最终响应更新
      console.log('[MainContent] Final response:', response.substring(0, 50));
      if (taskIdToUse) {
        setMessagesMap(prev => {
          const messages = prev[taskIdToUse!] || [];
          return {
      ...
      console.error('[MainContent] OpenClaw error:', error);
      // Show error message
      if (taskIdToUse) {
        setMessagesMap(prev => {
          const messages = prev[taskIdToUse!] || [];
          return {
            ...
            [taskIdToUse!]: messages.map(msg =>
              msg.id === assistantMessageId
                ? { ...msg, content: `连接 OpenClaw Gateway 失败: ${error instanceof Error ? error.message : '未知错误'}\n\n请确保 OpenClaw Gateway 正在运行 (ws://127.0.0.1:18789)` }
                : msg
            )
          };
        });
