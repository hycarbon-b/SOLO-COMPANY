...
export function ChatPanel({ messages, isTyping, inputValue, setInputValue, onSendMessage, messagesEndRef, taskTitle, onUpdateTitle, openclawConnected }: ChatPanelProps) {
    ...
    <div className="flex-1 flex flex-col bg-white h-full">
      ...
      {taskTitle && (
        <div className="px-6 py-4 flex items-center justify-between gap-2">
          ...
          {openclawConnected !== undefined && (
            <div className="flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-medium" 
                 style={{ 
                   backgroundColor: openclawConnected ? '#dcfce7' : '#fee2e2',
                   color: openclawConnected ? '#166534' : '#991b1b'
                 }}>
              <div className={`w-2 h-2 rounded-full ${openclawConnected ? 'bg-green-600' : 'bg-red-600'}`} />
              <span>{openclawConnected ? 'AI 已连接' : 'AI 未连接'}</span>
            </div>
          )}
        </div>
      )}
