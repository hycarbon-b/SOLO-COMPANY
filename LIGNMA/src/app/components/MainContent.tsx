...
import { sendToOpenClaw, isOpenClawConnected } from '../../services/openclawGateway';
...
export function MainContent({ onAddTask, currentTaskId, selectedMenu, tasks, onUpdateTaskTitle, onUpdateTaskStatus }: MainContentProps) {
  ...
  // Check OpenClaw connection status periodically
  useEffect(() => {
  ...
  const handleSendMessage = async () => {
    ...
    // Call OpenClaw Gateway for real AI response
    try {
      console.log('[MainContent] Sending message to OpenClaw:', inputValue.trim());
      
      const aiResponse = await sendToOpenClaw(
        inputValue.trim(),
        (text, accumulated) => {
          // Streaming callback - update assistant message in real-time
          console.log('[MainContent] Streaming update:', text.substring(0, 50));
          
          if (taskIdToUse) {
            setMessagesMap(prev => {
              const messages = prev[taskIdToUse!] || [];
              const lastMessage = messages[messages.length - 1];
              
              // If last message is from assistant and is being streamed, update it
              if (lastMessage && lastMessage.role === 'assistant' && lastMessage.id.startsWith('stream-')) {
      ...
      console.log('[MainContent] Received response from OpenClaw:', aiResponse.substring(0, 100));
      ...
      // Fallback to mock response if OpenClaw fails
      const assistantMessage: Message = {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        content: '抱歉，无法连接到AI服务。请确保OpenClaw Gateway正在运行（ws://127.0.0.1:18789）。\n\n错误信息：' + (error instanceof Error ? error.message : String(error)),
        timestamp: new Date(),
      };
